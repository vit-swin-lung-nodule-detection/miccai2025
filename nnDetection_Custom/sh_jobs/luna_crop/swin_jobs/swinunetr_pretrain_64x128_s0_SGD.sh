#!/bin/bash
#SBATCH --job-name=sw_sgd
#SBATCH --output=sbatch_log_new/swin_64_pretrain_joint_SGD_s0_%j.out
#SBATCH --nodes=1
...
nndet_train 027 -o exp.fold=0 train=swinunetr \
    ...
    model_cfg.encoder_kwargs.pretrained_path=/usr/bmicnas02/data-biwi-01/lung_detection/pretrain_swin/runs/64_128_128/model_epoch_20.pt \
    trainer_cfg.initial_lr=2e-4 \
    trainer_cfg.scheduler=cosine \
    trainer_cfg.optimizer=sgd \
    train.mode=resume \
    --sweep

#train.mode=resume \
#swinunetr_pretrain_64x128_s0_SGD.sh
# trainer_cfg.initial_lr=2e-4 \
# trainer_cfg.scheduler=cosine \ #confirm in the code
# trainer_cfg.optimizer: "adamw" # or "sgd" # add this to the config and check if it is in the code
# trainer_cfg.decoder_lr= \
