#!/bin/bash
#SBATCH --job-name=drop_out
#SBATCH --output=sbatch_log_new/mae16x224_s0_pretrain_joint_dropout%j.out
#SBATCH --nodes=1
...
nndet_train 026 -o exp.fold=0 \
    ...
    model_cfg.encoder_kwargs.drop=0.1 \
    model_cfg.encoder_kwargs.attn_drop=0.1 \
    model_cfg.encoder_kwargs.drop_path=0.1 \
    --sweep

# train.mode=resume \
#    trainer_cfg.scheduler=poly \
# mae16x224_submit0_pretrain_joint_dropout.sh
