#!/bin/bash
#SBATCH --job-name=gra_weight
#SBATCH --output=sbatch_log_new/mae16x224_s0_pretrain_joint_gradientweight%j.out
#SBATCH --nodes=1
...
nndet_train 020 -o exp.fold=0 \
    train=mae_plain +augment_cfg.patch_size=[16,224,224] \
    trainer_cfg.freeze_encoder=False \
    trainer_cfg.gradient_clip_val=10  trainer_cfg.max_num_epochs=60 \
    ...
    train.mode=resume \
    trainer_cfg.weight_decay=5.e-3  \
    --sweep

# train.mode=resume \
#    trainer_cfg.scheduler=poly \
#mae16x224_submit0_pretrain_joint_gradient_andweight.sh
