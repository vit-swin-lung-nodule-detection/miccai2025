...
from segment_lung import proces_and_segm
...
def _convert_data(f, target):
    ...
    try:
        ...
        image_array = sitk.GetArrayViewFromImage(data)
        
        # print(image_array.shape)      #z,x,y  
        # slice_nr = image_array.shape[0]//2
        # plt.imshow(image_array[slice_nr,:,:], cmap='gray')
        # plt.show()
        
        #include lung segmentation code here, exclude the resample and crop part 
        data_segm = proces_and_segm(data,f)
        # plt.imshow(data_segm[slice_nr,:,:], cmap='gray')
        # plt.show()

        if isinstance(data_segm, np.ndarray):
            data_segm = sitk.GetImageFromArray(data_segm)
            
                # Plot the slice from the NumPy array
            # data_segm_back_to_array = sitk.GetArrayViewFromImage(data_segm)
            # plt.imshow(data_segm_back_to_array[slice_nr, :, :], cmap='gray')
            # plt.title("After conversion to and from SimpleITK")
            # plt.show()

            sitk.WriteImage(data_segm, str(target / f"{f.stem.replace('.', '_')}_0000.nii.gz"))
    except Exception as e:
