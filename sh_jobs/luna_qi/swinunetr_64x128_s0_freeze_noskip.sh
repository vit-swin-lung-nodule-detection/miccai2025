#!/bin/bash
#SBATCH --job-name=qi_freeze
#SBATCH --output=sbatch_log_new/luna_crop_swin_64_int_joint_s0_freeze_noskip%j.out
#SBATCH --nodes=1
#SBATCH --time=48:00:00
#SBATCH --gres=gpu:1

#SBATCH --nodelist=bmicgpu07,bmicgpu09,octopus01,octopus02,octopus03,octopus04
...
nndet_train 018 -o exp.fold=0 \
    train=swinunetr_noskip \
    +trainer_cfg.freeze_encoder=True \
    +augment_cfg.patch_size=[64,128,128]  \
    trainer_cfg.gradient_clip_val=0  \
    trainer_cfg.max_num_epochs=60 trainer_cfg.swa_epochs=0 \
    trainer_cfg.warm_iterations=4000  \
    trainer_cfg.gradient_clip_val=0 trainer_cfg.amp_backend=None \
    ...
    train.mode=resume \
    --sweep
#swinunetr_64x128_s0_freeze_noskip.sh
