...
#SBATCH --output=sbatch_log/luna_crop_mae_16x224_s1_freeze_adamw_cosine_nndet_prep_decoderlr1e-2_%j.out
#SBATCH --nodes=1
...
# nndet_unpack ${det_data}/Task100_Luna_nndet_prep_int_16/preprocessed/D3V001_3d/imagesTr 6 --to_int 
...
nndet_train 100 -o exp.fold=0 train=mae train.mode=resume +augment_cfg.patch_size=[16,224,224] trainer_cfg.freeze_encoder=True trainer_cfg.decoder_lr=1e-4 trainer_cfg.gradient_clip_val=0  trainer_cfg.max_num_epochs=50 trainer_cfg.swa_epochs=10 trainer_cfg.warm_iterations=4000  trainer_cfg.gradient_clip_val=0 trainer_cfg.amp_backend=None trainer_cfg.precision=32 trainer_cfg.amp_level=None +trainer_cfg.scheduler=poly --sweep 
