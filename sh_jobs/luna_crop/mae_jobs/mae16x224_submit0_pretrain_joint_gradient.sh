#!/bin/bash
#SBATCH --job-name=gradient
#SBATCH --output=sbatch_log_new/mae16x224_s0_pretrain_joint_gradient%j.out
#SBATCH --nodes=1
...
nndet_train 052 -o exp.fold=0 \
    train=mae_plain +augment_cfg.patch_size=[16,224,224] \
    trainer_cfg.freeze_encoder=False \
    trainer_cfg.gradient_clip_val=4  trainer_cfg.max_num_epochs=60 \
...
#mae16x224_submit0_pretrain_joint_gradient.sh
